...
local n_maps = { -- {{{
  ...
  e = { name = 'edit..', -- {{{
    ...
    c = { name = 'Comment..', -- {{{
      b = {':norm ^mm%y^`mA // END <C-r>"<Esc>', 'close comment on Bracket'},
  ...
  p = { name = "Plugged..", -- {{{
    ...
    l = { name = 'LSP', -- {{{
      ...
      a = {"<cmd>lua vim.lsp.buf.code_action()<CR>", "code_action"},
      f = {"<cmd>lua vim.diagnostic.open_float()<CR>", "open Floate"},
      p = {'<cmd>lua vim.diagnostic.goto_prev({ border = "rounded" })<CR>', "previous"},
      l = {'<cmd>lua vim.lsp.diagnostic.show_line_diagnostics({ border = "rounded" })<CR>', "show Line"},
      n = {'<cmd>lua vim.diagnostic.goto_next({ border = "rounded" })<CR>', "Next"},
      L = {"<cmd>lua vim.diagnostic.setloclist()<CR>", "setLoclist"},
    }, -- }}}
...
local iQ_maps = { -- {{{
  ...
  m = { name = 'Marks <++>', -- {{{
    m = {'<Esc>/<++><Enter>va<"_c',    'next Mark'},
    p = {'<Esc>?<++><Enter>va<"_c',    'Previous mark'},
    r = {'<Esc>/<++><Enter>.',    'Repeat last action'},
  }, -- }}}
